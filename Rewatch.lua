...
function rewatch_AddPlayer(player, pet)
	...
	local frame = CreateFrame("FRAME", nil, rewatch_f, "BackdropTemplate");
	...
	-- Frames created with BackdropTemplate have SetBackdrop
	...
	frame:SetScript("OnMouseDown", function() if(not rewatch_loadInt["LockP"] and not InCombatLockdown()) then frame:StartMoving(); rewatch_f:SetBackdropColor(1, 0.49, 0.04, 1); end; end);
	frame:SetScript("OnMouseUp", function() if(not InCombatLockdown()) then frame:StopMovingOrSizing(); rewatch_f:SetBackdropColor(1, 0.49, 0.04, 0); rewatch_SnapToGrid(frame); end; end);

	-- build border frame with BackdropTemplate for MoP Classic compatibility
	...
	if(rewatch_loadInt["Layout"] == "default") then statusbar2:SetWidth(rewatch_loadInt["SpellBarWidth"] * (rewatch_loadInt["Scaling"]/100));
	...
	tgb:SetScript("OnMouseDown", function(_, button)
		...
		elseif(not rewatch_loadInt["LockP"] and not InCombatLockdown()) then frame:StartMoving(); rewatch_f:SetBackdropColor(1, 0.49, 0.04, 1); end;
	end);
	tgb:SetScript("OnMouseUp", function() if(not InCombatLockdown()) then frame:StopMovingOrSizing(); rewatch_f:SetBackdropColor(1, 0.49, 0.04, 0); rewatch_SnapToGrid(frame); end; end);
...
function rewatch_HidePlayer(playerId)
	...
	-- return if playerId or bars don't exist
	if(not rewatch_bars[playerId] or not rewatch_bars[playerId]["Frame"]) then return; end;
	
	-- remove the bar
	local parent = rewatch_bars[playerId]["Frame"]:GetParent();
	if(rewatch_bars[playerId]["PlayerBar"]) then rewatch_bars[playerId]["PlayerBar"]:Hide(); end;
	if(rewatch_bars[playerId]["PlayerBarInc"]) then rewatch_bars[playerId]["PlayerBarInc"]:Hide(); end;
	if(rewatch_bars[playerId][rewatch_loc["lifebloom"].."Bar"]) then rewatch_bars[playerId][rewatch_loc["lifebloom"].."Bar"]:Hide(); end;
	if(rewatch_bars[playerId][rewatch_loc["rejuvenation"].."Bar"]) then rewatch_bars[playerId][rewatch_loc["rejuvenation"].."Bar"]:Hide(); end;
	if(rewatch_bars[playerId][rewatch_loc["wildgrowth"].."Bar"]) then
			rewatch_bars[playerId][rewatch_loc["wildgrowth"].."Bar"]:Hide();
	end;
	if(rewatch_bars[playerId][rewatch_loc["regrowth"].."Bar"]) then rewatch_bars[playerId][rewatch_loc["regrowth"].."Bar"]:Hide(); end;
	...
	if(rewatch_bars[playerId]["Frame"]) then
	...
	if(parent and parent ~= UIParent) then rewatch_AlterFrame(); end;
end;
...
function rewatch_BuildFrame()
	-- create it with BackdropTemplate for MoP Classic compatibility
	rewatch_f = CreateFrame("FRAME", "Rewatch_Frame", UIParent, "BackdropTemplate");
	if(not rewatch_f) then
		error("Failed to create Rewatch frame!");
		return;
	end;
	...
	-- set looks (frames created with BackdropTemplate have SetBackdrop)
	...
	rewatch_f:SetScript("OnMouseDown", function(_, button)
		if(button == "RightButton") then
			if(rewatch_loadInt and rewatch_loadInt["Lock"]) then
				rewatch_loadInt["Lock"] = false; if(rewatch_OptionsFromData) then rewatch_OptionsFromData(true); end;
				if(rewatch_Message) then rewatch_Message(rewatch_loc["unlocked"]); end;
			else
				if(not rewatch_loadInt) then rewatch_loadInt = {}; end;
				rewatch_loadInt["Lock"] = true; if(rewatch_OptionsFromData) then rewatch_OptionsFromData(true); end;
				if(rewatch_Message) then rewatch_Message(rewatch_loc["locked"]); end;
			end;
		else 
			if(not rewatch_loadInt or not rewatch_loadInt["Lock"]) and not InCombatLockdown() then rewatch_f:StartMoving(); end; 
		end;
	end);
	rewatch_f:SetScript("OnMouseUp", function() if(not InCombatLockdown()) then rewatch_f:StopMovingOrSizing(); end; end);
	rewatch_f:SetScript("OnEnter", function () 
		rewatch_f:SetBackdropColor(1, 0.49, 0.04, 1); 
	end);
	rewatch_f:SetScript("OnLeave", function () 
		rewatch_f:SetBackdropColor(1, 0.49, 0.04, 0); 
	end);
...
function rewatch_SlashCommandHandler(cmd)
	-- ensure initialization has occurred before processing commands
	if(not rewatch_loadInt or not rewatch_loadInt["Loaded"]) then
		if(not rewatch_load) then
			rewatch_load = {};
		end;
		if(not rewatch_loadInt) then
			rewatch_loadInt = {};
		end;
		-- ensure frames are built before initialization
		if(not rewatch_f) then
			rewatch_BuildFrame();
		end;
		rewatch_OnLoad();
	...
	if(cmd) then
		...
		elseif(string.lower(commands[1]) == "default") then
			if(InCombatLockdown()) then rewatch_Message(rewatch_loc["combatfailed"]);
				...
				rewatch_load["Layout"] = "default";
				rewatch_load["SpellBarWidth"] = 85;
				rewatch_load["SpellBarHeight"] = 6;
				rewatch_load["HealthBarHeight"] = 30;
				rewatch_loadInt["Layout"] = "default";
				rewatch_loadInt["SpellBarWidth"] = 85;
				rewatch_loadInt["SpellBarHeight"] = 6;
				rewatch_loadInt["HealthBarHeight"] = 30;
				rewatch_changed = true;
				rewatch_DoUpdate();
				rewatch_Message("Layout set to default.");
			end;
		...
		elseif(string.lower(commands[1]) == "test") then
			if(InCombatLockdown()) then rewatch_Message(rewatch_loc["combatfailed"]);
				...
				for i, name in ipairs(testPlayers) do
					if(rewatch_GetPlayer(name) < 0) then
						rewatch_AddPlayer(name, nil);
					end;
				end;
				rewatch_Message("Added "..#testPlayers.." test players. Cast healing spells on yourself to see the timers update.");
			end;
		...
		elseif(string.lower(commands[1]) == "forcedheight") then
			if(tonumber(commands[2])) then
				...
				rewatch_loadInt["NumFramesWide"] = 1;
				rewatch_load["NumFramesWide"] = 1;
				rewatch_Message("Forced height to "..rewatch_load["ForcedHeight"]..". Set to 0 to set to autosizing."); 
				if(rewatch_f) then rewatch_AlterFrame(); end;
			end;
		...
		elseif(string.lower(commands[1]) == "gcdalpha") then
			...
			elseif((tonumber(commands[2]) < 0) or (tonumber(commands[2]) > 1)) then rewatch_Message(rewatch_loc["nonumber"]);
				...
				if(rewatch_gcd) then rewatch_gcd:SetAlpha(rewatch_loadInt["GcdAlpha"]); end;
				rewatch_OptionsFromData(true);
		...
		elseif(string.lower(commands[1]) == "hidesolo") then
			if(not((commands[2] == "0") or (commands[2] == "1"))) then rewatch_Message(rewatch_loc["nonumber"]);
				...
				if(rewatch_f) then
					if(((rewatch_i == 2) and (rewatch_load["HideSolo"] == 1)) or ((rewatch_load["Hide"] or 0) == 1)) then rewatch_f:Hide(); else rewatch_ShowFrame(); end;
				end;
				rewatch_OptionsFromData(true);
		...
		elseif(string.lower(commands[1]) == "hide") then
			if(not rewatch_load) then rewatch_load = {}; end;
			if(not rewatch_loadInt) then rewatch_loadInt = {}; end;
			rewatch_load["Hide"] = 1; rewatch_loadInt["Hide"] = rewatch_load["Hide"];
			if(rewatch_f) then
				if(((rewatch_i == 2) and ((rewatch_load["HideSolo"] or 0) == 1)) or (rewatch_load["Hide"] == 1)) then rewatch_f:Hide(); else rewatch_ShowFrame(); end;
			end;
...
rewatch_events:SetScript("OnEvent", function(self, event, ...)
	-- only process if properly loaded
	if(not rewatch_loadInt["Loaded"]) then
		return;
	end;
	
	-- Handle non-combat-log events first
	...
	elseif(event == "COMBAT_LOG_EVENT_UNFILTERED") then
		-- In MoP Classic, use CombatLogGetCurrentEventInfo() to get combat log data
		local timestamp, combatEvent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceFlags2, destGUID, destName, destFlags, destFlags2, spellId, spellName, spellSchool, amount, overkill, school, resisted, blocked, absorbed, critical, glancing, crushing, isOffHand = CombatLogGetCurrentEventInfo();
		
		-- Map combat log events to our old parameter names for compatibility
		local unitGUID = sourceGUID;
		local effect = combatEvent;
		...
		-- For AURA_APPLIED_DOSE events, we need to extract stack count from amount
		if(combatEvent == "SPELL_AURA_APPLIED_DOSE") then
			-- Ensure amount is converted to a number (it might be a string)
			stacks = tonumber(amount) or amount; -- In DOSE events, amount is the stack count
		end;
		
		--print("combat event ", timestamp, combatEvent, sourceGUID, destName, spellName, spellSchool, stacks);
		...
		elseif((spell == rewatch_loc["clearcasting"])) then
			...
			--		val[rewatch_loc["regrowth"].."Bar"]:-highlight-();
			--	else break end;
			--end; end;
	...
	elseif((effect == "SPELL_AURA_REMOVED") or (effect == "SPELL_AURA_DISPELLED") or (effect == "SPELL_AURA_REMOVED_DOSE")) then
		...
		elseif((spell == rewatch_loc["clearcasting"])) then
			--for i=1,rewatch_i-1 do val = rewatch_bars[i]; if(val) then
			--	if(val[rewatch_loc["regrowth"]]) then
			--		val[rewatch_loc["regrowth"].."Bar"]:-unhighlight-();
			--	else break end;
			--end; end;
	...
	elseif((effect == "SPELL_CAST_SUCCESS") or (effect == "SPELL_HEAL")) then
		...
		if(meGUID == UnitGUID("player")) then
			...
			-- fix bug where the 3rd stack refresh of LB doesn't trigger SPELL_AURA_REFRESH
			elseif((spell == rewatch_loc["lifebloom"]) and rewatch_InGroup(targetName) and (effect == "SPELL_CAST_SUCCESS")) then
				rewatch_UpdateBar(rewatch_loc["lifebloom"], targetName, nil);
			-- resolves refresh of LB by a big heal (Regrowth's instant heal, Healing Touch, Nourish)
			-- Check both SPELL_CAST_SUCCESS (cast completes) and SPELL_HEAL (heal actually lands)
			-- In MoP Classic, big direct heals refresh Lifebloom regardless of Blooming glyph
			elseif(((spell == rewatch_loc["regrowth"]) or (spell == rewatch_loc["healingtouch"]) or (spell == rewatch_loc["nourish"])) and rewatch_InGroup(targetName) and ((effect == "SPELL_CAST_SUCCESS") or (effect == "SPELL_HEAL"))) then
				-- Only refresh if Lifebloom exists on the target
				local lbPlayerId = rewatch_GetPlayer(targetName);
				if(lbPlayerId > 0 and rewatch_bars[lbPlayerId] and rewatch_bars[lbPlayerId][rewatch_loc["lifebloom"]]) then
					rewatch_UpdateBar(rewatch_loc["lifebloom"], targetName, nil);
				end;
			end;
		end;
	...
	elseif((effect == "SPELL_CAST_START") and ((spell == rewatch_loc["rebirth"]) or (spell == rewatch_loc["revive"])) and (meGUID == UnitGUID("player"))) then
		...
		elseif((effect == "SPELL_RESURRECT") and (spell == rewatch_loc["rebirth"]) and (meGUID == UnitGUID("player"))) then
			...
			if(rewatch_loadInt["ShowButtons"] == 1) then
				for i=1,rewatch_i-1 do val = rewatch_bars[i]; if(val) then
					if(val["NourishButton"]) then
						val["NourishButton"].doUpdate = true;
					else break end;
				end; end;
			end;
		end; -- end of combat log event handling
	end; -- end of OnEvent function
end);
		...
		for i=1,rewatch_i-1 do val = rewatch_bars[i];
			...
			if(val) then
				...
				if(UnitIsDeadOrGhost(val["Player"])) then
					...
					if(val["Mark"]) then
						safeSetBackdropColor(val["Frame"], rewatch_loadInt["MarkFrameColor"].r, rewatch_loadInt["MarkFrameColor"].g, rewatch_loadInt["MarkFrameColor"].b, rewatch_loadInt["MarkFrameColor"].a);
					else
						safeSetBackdropColor(val["Frame"], rewatch_loadInt["FrameColor"].r, rewatch_loadInt["FrameColor"].g, rewatch_loadInt["FrameColor"].b, rewatch_loadInt["FrameColor"].a);
					end;
						val["LifebloomStacks"] = 0;
					...
					if(val["NourishButton"] and val["NourishButton"].doUpdate == true) then
						local start, duration = GetSpellCooldown(rewatch_loc["rebirth"]);
						if(val["NourishButton"].cooldown and val["NourishButton"].cooldown.SetCooldown) then
							val["NourishButton"].cooldown:SetCooldown(start, duration);
						elseif(CooldownFrame_SetTimer) then
							CooldownFrame_SetTimer(val["NourishButton"].cooldown, start, duration);
						end;
						val["NourishButton"].doUpdate = false;
					end;
					if(val["RemoveCorruptionButton"] and val["RemoveCorruptionButton"].doUpdate == true) then
						local start, duration = GetSpellCooldown(val["RemoveCorruptionButton"].spellName);
						if(val["RemoveCorruptionButton"].cooldown and val["RemoveCorruptionButton"].cooldown.SetCooldown) then
							val["RemoveCorruptionButton"].cooldown:SetCooldown(start, duration);
						elseif(CooldownFrame_SetTimer) then
							CooldownFrame_SetTimer(val["RemoveCorruptionButton"].cooldown, start, duration);
						end;
					...
					if(val["SwiftmendButton"] and val["SwiftmendButton"].doUpdate == true) then
						local start, duration = GetSpellCooldown(rewatch_loc["swiftmend"]);
						if(val["SwiftmendButton"].cooldown and val["SwiftmendButton"].cooldown.SetCooldown) then
							val["SwiftmendButton"].cooldown:SetCooldown(start, duration);
						elseif(CooldownFrame_SetTimer) then
							CooldownFrame_SetTimer(val["SwiftmendButton"].cooldown, start, duration);
						end;
						val["SwiftmendButton"].doUpdate = false;
					end;
					if(val["ThornsButton"] and val["ThornsButton"].doUpdate == true) then
						local start, duration = GetSpellCooldown(val["ThornsButton"].spellName);
						if(val["ThornsButton"].cooldown and val["ThornsButton"].cooldown.SetCooldown) then
							val["ThornsButton"].cooldown:SetCooldown(start, duration);
						elseif(CooldownFrame_SetTimer) then
							CooldownFrame_SetTimer(val["ThornsButton"].cooldown, start, duration);
						end;
					...
					if(rewatch_bars[i][rewatch_loc["lifebloom"]] > 0) then
						...
						if(left > 0) then
							...
							-- Update stack count from UnitBuff to ensure color is correct
							local _, _, stackCount = findUnitBuff(val["Player"], rewatch_loc["lifebloom"], "PLAYER");
							if(stackCount and stackCount > 0) then
								rewatch_bars[i]["LifebloomStacks"] = stackCount;
							end;
							
							-- Set color based on stack count
							local stackColorSuffix = "";
							local currentStacks = rewatch_bars[i]["LifebloomStacks"] or 1;
							if(currentStacks >= 3) then
								stackColorSuffix = "3";
							elseif(currentStacks >= 2) then
								stackColorSuffix = "2";
							...
							-- Only override with low-timer warning if very close to expiring (within 0.5 seconds)
							if(math.abs(left-0.5) < 0.2) then 
								val[rewatch_loc["lifebloom"].."Bar"]:SetStatusBarColor(0.6, 0.0, 0.0, 1);
							else
								-- Use stack-based color
								local colorKey = "BarColor"..rewatch_loc["lifebloom"]..stackColorSuffix;
								if(rewatch_loadInt[colorKey]) then
									val[rewatch_loc["lifebloom"].."Bar"]:SetStatusBarColor(rewatch_loadInt[colorKey].r, rewatch_loadInt[colorKey].g, rewatch_loadInt[colorKey].b, rewatch_loadInt[colorKey].a);
								end;
							end;
						elseif(left < -1) then
							rewatch_DowndateBar(rewatch_loc["lifebloom"], i);
