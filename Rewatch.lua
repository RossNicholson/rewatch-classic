...
function rewatch_UpdateBar(spellName, player, stacks)
	...
	if(rewatch_bars[playerId][spellName]) then
		...
		local c = "";
		if(spellName == rewatch_loc["lifebloom"]) then
			if(not rewatch_bars[playerId]["LifebloomStacks"]) then stacks = 1; end;
			if(stacks) then rewatch_bars[playerId]["LifebloomStacks"] = stacks; end;
			c = rewatch_bars[playerId]["LifebloomStacks"]; if(c <= 1) then c = ""; end;
			...
			if((rewatch_bars[playerId]["LifebloomStacks"] == 3) and (stacks == nil)) then
				if(rewatch_loadInt["HasBlooming"]) then b = 10; else b = 15; end;
				a = GetTime() + b;
			end;
		end;
			...
			if(val) then
				...
				if(UnitIsDeadOrGhost(val["Player"])) then
					...
					if(val["NourishButton"] and val["NourishButton"].doUpdate == true) then
						CooldownFrame_SetTimer(val["NourishButton"].cooldown, GetSpellCooldown(rewatch_loc["rebirth"]));
